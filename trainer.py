...
def validate(model, vocab, val_data, n_batch=32):
    ...
    for batched_data in batcher(val_data, n=n_batch):
        ...
        token_corrects += ((predictions == targets) * (targets != model.tokenizer.pad_token_id)).sum().item()
        token_totals += (targets != model.tokenizer.pad_token_id).sum().item()
        val_loss += loss.item()
        total_items += 1
    ...
    for i in tqdm(range(N_EPOCH)):
        ...
        for batched_data in pbar:
            ...
            if iter % validate_every == 0:
                ...
                with torch.no_grad():
                    val_loss, token_accuracy = validate(model, vocab, val_data, n_batch=params["n_batch"])
                    ...
                    predictions = model.tokenizer.batch_decode(predictions, skip_special_tokens=True)
                    inp, out = batch
